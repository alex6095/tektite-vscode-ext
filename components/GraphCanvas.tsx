...
export const GraphCanvas: React.FC<GraphCanvasProps> = ({ 
  ...
  useEffect(() => {
     ...
     svg.selectAll('.link')
        ...
        .attr('stroke', (d: any) => {
            if (d.type === EdgeType.CALLS) return '#6E8FEE';
            if (d.type === EdgeType.IMPORTS) return '#4ADE80';
            if (d.type === EdgeType.SEMANTIC) return '#A78BFA'; 
            return '#555';
        })
  ...
  return (
    <div 
      ref={containerRef} 
      className="w-full h-full relative overflow-hidden bg-[#0F1117] cursor-grab active:cursor-grabbing"
      onClick={handleBgClick}
    >
      <div 
        className="absolute inset-0 opacity-20 pointer-events-none"
        style={{
            backgroundImage: 'radial-gradient(circle at 2px 2px, #363B49 1px, transparent 0)',
      ...
      <svg ref={svgRef} className="absolute inset-0 w-full h-full pointer-events-none">
        <defs>
           ...
           <marker id={`arrow-${EdgeType.CALLS}`} viewBox="0 -5 10 10" refX="35" refY="0" markerWidth="6" markerHeight="6" orient="auto"><path d="M0,-5L10,0L0,5" fill="#6E8FEE" /></marker>
           <marker id={`arrow-${EdgeType.IMPORTS}`} viewBox="0 -5 10 10" refX="35" refY="0" markerWidth="6" markerHeight="6" orient="auto"><path d="M0,-5L10,0L0,5" fill="#4ADE80" /></marker>
        </defs>
      ...
      <div className="absolute bottom-6 left-6 glass-panel p-4 rounded-xl pointer-events-auto transition-opacity duration-300 hover:opacity-100 opacity-60">
          <h3 className="text-xs font-bold text-gray-400 uppercase mb-2">Graph Legend</h3>
          <div className="space-y-1.5 text-xs">
              <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-[#6E8FEE] mr-2"></div>calls (Function Call)</div>
              <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-[#4ADE80] mr-2"></div>imports (Belongs To)</div>
              {(layoutMode === 'semantic' || edgeFilters.showSemantic) && (
                  <div className="flex items-center"><div className="w-3 h-3 rounded-full bg-[#A78BFA] mr-2"></div>semantic (AI Similarity)</div>
              )}
          </div>
      </div>
    </div>
